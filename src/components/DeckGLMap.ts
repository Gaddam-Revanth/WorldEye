...
export class DeckGLMap {
  ...
  private satelliteRiskPoints: Array<{ lat: number; lon: number; risk: number; type?: string }> = [];
  ...
  private buildLayers(): LayersList {
    ...
    // Satellite risk layer
    if (mapLayers.satellite) {
      const satLayer = this.createSatelliteRiskLayer();
      if (satLayer) layers.push(satLayer);
    }
  ...
  private createSatelliteRiskLayer(): HeatmapLayer | null {
    if (this.satelliteRiskPoints.length === 0) return null;

    return new HeatmapLayer({
      id: 'satellite-risk-layer',
      data: this.satelliteRiskPoints,
      getPosition: (d: { lat: number; lon: number }) => [d.lon, d.lat],
      getWeight: (d: { risk: number }) => d.risk,
      radiusPixels: 60,
      intensity: 1.5,
      threshold: 0.1,
      opacity: 0.7,
  ...
  private createLayerToggles(): void {
    ...
    const layerConfig = SITE_VARIANT === 'tech'
      ? [
        ...
        { key: 'satellite', label: 'Satellite Risk', icon: '🛰️' },
      ]
  ...
  public setSatelliteRiskPoints(points: Array<{ lat: number; lon: number; risk: number; type?: string }>): void {
    this.satelliteRiskPoints = points;
    this.render();
  }
