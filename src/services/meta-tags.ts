// Dynamic Meta Tags Service for WorldEye
...
export function updateMetaTagsForStory(meta: StoryMeta): void {
  ...
  const title = `${countryName} Intelligence Brief | WorldEye`;
...
export function resetMetaTags(): void {
  const defaultTitle = 'WorldEye - Global Situation with AI Insights';
  const defaultDesc = 'WorldEye: AI-powered real-time global intelligence dashboard with live news, markets, military tracking, and geopolitical data.';
...
function generateDescription(
  ...
  return `WorldEye ${parts.join('. ')}. Free, open-source geopolitical intelligence.`;
}
